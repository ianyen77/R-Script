...
data<-read.csv("C:/Users/USER/Desktop/rel_abundance_table/families.csv",header=T)
...
data2<-rbind(data,dd)
#要幾個階層就放幾個
rownames(data2)[length(data2$M1.S)]<-"other;Others;Others;Others; Others"
data3<-data2[apply(data2, 1, function(x) !all(x==0)),]
data3$phyla<-rownames(data3)
rownames(data3)<-NULL
data4<-gather(data =data3, key ="xx",value = "yf",1:15)
data4<-separate(data4,phyla,into=c("domain","phyla","CLASS","Order","Family"),sep=";")
